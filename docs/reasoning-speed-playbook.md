...
This lab defaults important CLI work to `gpt-5.5` with `xhigh` reasoning. That improves hard-case reasoning, but it is not the fastest path for every step. The goal is to keep correctness while reducing wall-clock latency and avoidable token spend.
...
- Use bounded specialists for lookup: fast explore/research passes should return compact facts, not full transcripts.
