...
- Root lab positioning has been corrected: the sandbox is scenario-neutral and can host arbitrarily large agent families. UCP and commercial customer-service are future scenario workspaces only. The lab's job is to amplify Codex and Claude with durable state, harnesses, Waterflow, benchmarks, skills, and verification gates, not replace their reasoning or coding ability.
...
- Workspace safety gate is installed: `scripts/check-workspace-safety` checks all `workspaces/*` for hard safety failures while allowing active in-progress scaffolding gaps as warnings. It writes `outputs/shared/workspace-safety/workspace-safety.json` and `.md`, is wired into `scripts/check-lab`, `scripts/check-project-rules`, `scripts/check-async-execution`, and `scripts/lab-dashboard`, and keeps active Claude/Codex workspace creation from breaking the default lab health loop.
...
- Collaboration honest status: commands are installed (`omc ask`, `omc team`, `omc interop`, Codex `omx-api`), but real interop is unproven. The earlier OMC team run is recorded `blocked` (`worker_start_submit_unverified`), and assignment `collab-0003-interop-proof` stays `pending` until a real tmux run from a terminal produces an `outputs/shared/` artifact.
